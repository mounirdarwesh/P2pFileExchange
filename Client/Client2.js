...
const folderHandler = require('./folderHandler copy')
...
// sender is Username
let sender = readFileSync(path.join(__dirname, 'file_exchange - Copy', 'ID.txt'), 'utf8',
  function (err, data) {
    if (err) {
...
let sortedFiles = folderHandler(path.join(__dirname, 'file_exchange - Copy/sendData'))
